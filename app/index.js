...
const writePostsToDb = ({ forumTitle, posts }) => {
  if (!Array.isArray(posts) || !forumTitle) {
    throw new TypeError(
      'Expected posts to be Array and forumTitle to be defined'
    )
  }
  ...
  pool.connect((err, client, done) => {
    ...
    client.query(sqlQuery, [threadIds], (err, result) => {
      ...
      postsToUpdate = posts.filter(
        ep =>
          result.rows.filter(
            p => p.threadId === ep.threadId && ep.updateDate < p.updateDate
          ).length > 0
      )
      postsToInsert = posts.filter(
        p => result.rows.filter(ep => ep.threadId === p.threadId).length <= 0
      )
...
const writePostCollectionToDb = postCollection => {
  if (!Array.isArray(postCollection)) {
    throw new TypeError(
      `Expected postCollection to be Array, received ${typeof posts}`
    )
  }
